...
def send_low_stock_email(ind_df, grp_df):
    sender_email = st.secrets["email_sender"]
    sender_password = st.secrets["email_password"]
    recipient_emails = ["s.tufo@imeva.com", "e.fallarino@imeva.com"] 
    
    # Convert low-stock DataFrames into HTML tables CLEANNNN
    if not ind_df.empty:
        ind_html = ind_df[['lotto_number', 'grade', 'thickness', 'active_w']].to_html(index=False, border=1)
    else:
        ind_html = "<p>No individual lots below threshold.</p>"
        
    if not grp_df.empty:
        grp_html = grp_df.to_html(index=False, border=1)
    else:
        grp_html = "<p>No material groups below threshold.</p>"
    
    msg = MIMEMultipart("alternative")
    msg["Subject"] = "⚠️ Low Stock Alert - IMEVA Material Hub"
    ...
    html_content = f"""
        <html>
          <body style="font-family: Arial, sans-serif;">
            <h3 style="color: #d9534f;">⚠️ Low Stock Warning</h3>
            <p>The following materials have reached or fallen below the 20 kg threshold in the production batch queue:</p>
            
            <h4 style="color: #333;">Individual Lots:</h4>
            {ind_html}
            
            <h4 style="color: #333; margin-top: 20px;">Aggregated Groups (Grade & Thickness):</h4>
            {grp_html}
            
            <p style="margin-top: 30px; font-size: 12px; color: #777;">
              Automated message generated by IMEVA Material Hub.
            </p>
          </body>
        </html>
    """
    ...
    # Connect to Outlook SMTP server (Port 587 with TLS)
    try:
        ...
        raise Exception(f"Outlook SMTP Error: {str(e)}")
...
# =========================================================================
    ...
    if not prod_df.empty:
        temp_w = prod_df['rd_remaining_weight_kg'].fillna(prod_df['coil_weight_kg'])
        
        # Filter individual lots between 0.001 and 20 kg
        low_stock_individual = prod_df[(temp_w > 0.001) & (temp_w <= 20.0)].copy()
        low_stock_individual['active_w'] = temp_w
        
        # Aggregated groups by grade and thickness
        agg_df = prod_df.copy()
        agg_df['active_w'] = temp_w
        grouped = agg_df.groupby(['grade', 'thickness'])['active_w'].sum().reset_index()
        low_stock_groups = grouped[(grouped['active_w'] > 0.001) & (grouped['active_w'] <= 20.0)]
        
        # If anything is low, build a clear bulleted list of the exact materials
        if not low_stock_individual.empty or not low_stock_groups.empty:
            
            # Build a text list of the individual lots for the warning box
            material_bullets = ""
            for _, row in low_stock_individual.iterrows():
                lotto = row.get('lotto_number', 'N/A')
                grade = row.get('grade', 'N/A')
                thickness = row.get('thickness', 'N/A')
                weight = row.get('active_w', 0)
                material_bullets += f"\n- **Lot:** `{lotto}` | **Grade:** {grade} | **Thick:** {thickness} mm | **Weight:** {weight:.2f} kg"

            st.warning(f"""
    ⚠️ **Low-Stock Alert:** {len(low_stock_individual)} individual lot(s) are at or below the 20 kg threshold:
    {material_bullets}
            """)
            
            # Manual test button right on the page
            if st.button("🧪 Test Send Low-Stock Email Now"):
                try:
                    send_low_stock_email(low_stock_individual, low_stock_groups)
                    st.success("Test email dispatched successfully! Check your inbox.")
